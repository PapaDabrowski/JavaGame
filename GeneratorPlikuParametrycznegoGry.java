 ...
 * zawiera kilka błędów logicznych i antywzorców, a być może i nie tylko takie błędy.
...
public class GeneratorPlikuParametrycznegoGry {
	...
	void zapiszPlikParametryczny(String nazwaPliku) {
		...
		pw.println("# Plik parametryczny gry." + lsep
				...
				+ "#" + lsep
				+ "nazwaGry=" + losowo(nazwyGier) + " [" + pin + "]" + lsep //MAM
				...
				+ "#" + lsep
				+ "liczbaPoziomów=" + (3 + (p % 3)) + lsep //MAM
				+ "# dodatnia liczba całkowita" + lsep
				+ "nazwaBazowaPlikuZOpisemPoziomu=" + wgPinu(nazwyPlikówPoziomów) + lsep //MAM
				+ "numeracjaPoziomówZaczynaSięOd=" + wgPinu(0, 1) + lsep //MAM
				+ "# liczba 0 lub 1" + lsep
				+ "rozszerzeniePlikuZOpisemPoziomu=" + wgPinu(rozszerzenia) + lsep //MAM
				+ "#" + lsep
				+ "# 2. Stopnie trudności" + lsep
				+ "#" + lsep
				+ "liczbaStopniTrudności=" + losowo(2, 5) + lsep  //MAM
				+ "# dodatnia liczba całkowita" + lsep
				+ "zmianaStopniaTrudności=" + losowo(20, 45) + lsep //MAM
				...
				+ "#" + lsep
				+ "początkowaSzerokośćPlanszy=" + losowo(400, 1000) + lsep //MAM
				+ "początkowaWysokośćPlanszy=" + losowo(250, 700) + lsep //MAM
				+ "początkowaSzerokośćObiektuGryJakoProcentPoczątkowejSzerokościPlanszy=" //MAM
				+ (float) losowo(5d, 9d) + lsep
				+ "#" + lsep
				+ "# 4. Rysowanie tła i obiektów, do których się strzela" + lsep
				+ "#");
		if (losowo(0, 1) == 0) { // tło jednolite MAM
			pw.println("tło=jednolite" + lsep
					+ "kolorTła=" + losowo(0, 255) + " " + losowo(0, 255) + " " + losowo(0, 255) + lsep
					+ "# składowe R G B koloru tła, każda w zakresie 0..255");
		} else { // tło graficzne
			pw.println("tło=plikGraficzny" + lsep
		...
		pw.println("#" + lsep
		    + "#" + lsep
				+ "# 5.Wytrzymałość obiektów w sensie HP" + lsep
				+ "HitPoints =" + losowo(50,130) +lsep
				+ "#");

		pw.println("#" + lsep
				    + "#" + lsep
						+ "# 6.Liczba wyświetlanych wyników w tabeli wyników" + lsep
						+ "LiczbaWynikow =" + losowo(30,50) +lsep
						+ "#");
		pw.close();
	}
